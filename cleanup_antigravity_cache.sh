    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo "Options:"
                echo "  -q, --quick, -a, --all    Clean everything without interactive menu"
...
# Clean directory with animation and dry run support
clean_dir() {
    local dir="$1"
    local description="$2"
    ...
    if [ -d "$dir" ] && [ "$(ls -A "$dir" 2>/dev/null)" ]; then
        ...
        if [ "$DRY_RUN" -eq 1 ]; then
            printf "  ${YELLOW}${ICON_WARN}${RST} ${DIM}Would clean ${description}...${RST}"
            # Show file count or first few files?
            local count=$(find "$dir" -type f | wc -l)
            printf "\r  ${YELLOW}${ICON_WARN}${RST} ${WHITE}${description}${RST} ${GRAY}─${RST} "
            format_size "$size"
            printf " ${YELLOW}(${count} files would be removed)${RST}\n"
            return 0
        fi
...
draw_header() {
    if [ "$QUICK_MODE" -eq 1 ]; then return; fi
    clear
    hide_cursor
    printf "\n"
...
# Confirmation dialog
confirm_dialog() {
    # If using quick mode or dry run, likely skip confirmation or handle differently
    # But usually quick mode implies "yes"
    if [ "$QUICK_MODE" -eq 1 ]; then return 0; fi
...
perform_clean() {
    ...
    case $choice in
        ...
        7)
            ...
            ;;
        8)
            if [ "$QUICK_MODE" -eq 1 ] || [ "$DRY_RUN" -eq 1 ]; then
                 print_warning "Interactive tools skip in automode"
            else
                network_optimizer
            fi
            ;;
...
main() {
    ...
    if [ ! -d "$GEMINI_DIR" ]; then
        if [ "$QUICK_MODE" -eq 0 ]; then draw_header; fi
    ...
    if [ "$QUICK_MODE" -eq 1 ]; then
        ...
        # Default to clean all (option 6) in quick mode
        perform_clean "6"
        exit 0
    fi

    # Dry run check for interactive
    if [ "$DRY_RUN" -eq 1 ]; then
         print_info "Running in DRY RUN mode (no files will be deleted)"
         sleep 1
    fi
